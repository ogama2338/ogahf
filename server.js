...
const fs = require('fs');
const { pathToFileURL } = require('url'); // FIX: Added to prevent RAM Memory Crashes
...
// FIX: Increased size limits to prevent server blocking large uploads
...
// FIX: Added a generous fileSize limit to Multer
const upload = multer({ 
  dest: tempUploadsDir,
  limits: { fileSize: 2000 * 1024 * 1024 } // Allow files up to 2GB
});
...
app.post('/api/hf/upload-multiple', upload.array('files'), async (req, res) => {
  ...
  try {
    ...
    // FIX: Removed Memory-heavy Buffers and Blobs. Using direct file streaming URL.
    const filesToUpload = tempFiles.map((f, i) => {
        return {
            path: paths[i] || f.originalname,
            content: pathToFileURL(f.path) // Streams from disk directly! No memory crashes.
        };
    });
    ...
    // FIX: This will now properly send the exact Hugging Face error back to your browser
